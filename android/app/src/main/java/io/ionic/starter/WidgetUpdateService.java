...
public class WidgetUpdateService extends JobIntentService {
    ...
    private static final String EXTRA_FAVORITE_ID = "favorite_id";
    ...
    public static void enqueueWork(Context context, String favoriteId) {
        Intent intent = new Intent(context, WidgetUpdateService.class);
        if (favoriteId != null) {
            intent.putExtra(EXTRA_FAVORITE_ID, favoriteId);
        }
        enqueueWork(context, WidgetUpdateService.class, JOB_ID, intent);
    }
    ...
    protected void onHandleWork(@NonNull Intent intent) {
        ...
        String forcedFavoriteId = intent.getStringExtra(EXTRA_FAVORITE_ID);
        // Si forcedFavoriteId es una cadena vacía, es una acción explícita de "limpiar"
        // Si es null, significa que no se pasó nada, así que leer de preferencias
        boolean isForcedFavorite = forcedFavoriteId != null;
        String favoriteId = isForcedFavorite ? forcedFavoriteId : readFavoriteId(this);
        ...
        String initialFavoriteId = favoriteId;
        ...
        try {
            ...
            for (int i = 0; i < rotations; i++) {
                if (!isForcedFavorite) {
                    String currentFavoriteId = readFavoriteId(this);
                    if (currentFavoriteId == null || currentFavoriteId.isEmpty()) {
                        showEmptyState(manager, ids);
                        return;
                    }
                    if (!currentFavoriteId.equals(initialFavoriteId)) {
                        WidgetUpdateService.enqueueWork(this, currentFavoriteId);
                        return;
                    }
                }
    ...
    private void updateWidget(AppWidgetManager manager, int appWidgetId, GameData data,
        ...
        if (bg != null) {
            ...
            views.setImageViewResource(R.id.widget_bg, R.drawable.widget_bg_placeholder);
        }
    ...
    private void showEmptyState(AppWidgetManager manager, int[] ids) {
        for (int id : ids) {
            ...
            views.setTextViewText(R.id.widget_empty, "Marca un favorito en la app");
